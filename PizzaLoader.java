...
public class PizzaLoader extends Loader {
    ...
    private static final List<PizzaTopping> PEPPERONIS = List.of(
            new PizzaTopping(1.2, 0.0),
            new PizzaTopping(2.0, -1.1),
            new PizzaTopping(2.3, 1.25),
            new PizzaTopping(2.6, -0.3),
            new PizzaTopping(3.2, -1.75),
            new PizzaTopping(3.1, 1.7),
            new PizzaTopping(3.3, 0.2),
            new PizzaTopping(2.0, 0.5),
            new PizzaTopping(0.6, 0.0),
            new PizzaTopping(2.7, 0.9),
            new PizzaTopping(2.7, -0.9));
    ...
    protected void renderGeometry(String[] outputBuffer, double[] zBuffer) {
        ...
        for (double x = 0.0; x <= maxX; x += 0.06) {
            ...
            for (double y = -maxY; y <= maxY; y += 0.04) {
                ...
                // Bottom Crust Layer
                drawPoint(x, y, -CHEESE_THICKNESS / 2, 0.0, 0.0, -1.0, sinA, cosA, sinB, cosB, outputBuffer, zBuffer,
                        0);
            }
        }

        // --- STEP 2: Render Pepperoni Slices FIRST (Allows them to override depth
        // boundaries cleanly) ---
        for (PizzaTopping pep : PEPPERONIS) {
            for (double pr = 0.0; pr <= PEPPERONI_RADIUS; pr += 0.04) {
                for (double pa = 0; pa < 2 * Math.PI; pa += 0.2) {
                    ...
                    // FIXED: Bumped Z slightly higher to guarantee it overrides the cheese base
                    // depth
                    double pz = (CHEESE_THICKNESS / 2.0) + 0.05;

                    // Clipping Intersections
                    if ((px * px + py * py) > (MAX_RADIUS * MAX_RADIUS))
                        continue;
                    if (px < 0 || Math.abs(py) > (px * EDGE_SLOPE))
                        continue;
