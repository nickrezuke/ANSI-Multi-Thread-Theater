public class WireframeFisheyeGlobeLoader extends Loader {
    ...
    public WireframeFisheyeGlobeLoader() {
    ...
    protected void renderGeometry(String[] outputBuffer, double[] zBuffer) {
        ...
        // Step 1: Sample across the sphere's surface geometry (Latitude/Longitude
        // coordinates)
        ...
        for (int latStep = 0; latStep <= 60; latStep++) {
            ...
            for (int lonStep = 0; lonStep < 160; lonStep++) {
                ...
                // Translate sphere right in front of the lens path with a gaussian zoom
                double finalZ = rz + 1.8 + Math.exp(-Math.pow((1 + Math.sin(System.currentTimeMillis() / 1000.0))/2, 2) / 1.8);
                ;
                if (finalZ <= 0.1)
                    continue;
                ...
                // MAXED OUT FISHEYE: Exponent parameter set to 3.25 for dramatic barrel
                // distortion
