// "A" variant of the Utah Teapot loader: the exact 306 control points and
// 32 bicubic Bezier patches Martin Newell measured off a real Melitta
// teapot in 1975 and Jim Blinn later rescaled -- the same dataset baked
// into glutSolidTeapot() and every OpenGL textbook demo since -- evaluated
// with real De Casteljau/Bernstein math into a dense point cloud and
// plotted one character per point, no filled surfaces.
//
// Unlike SuzanneTheMonkeyLoaderA / StanfordDragonLoaderA / StanfordBunnyLoaderA,
// this was never a scanned or unstructured point cloud in need of PCA
// normal estimation: every point below already carries its own exact,
// analytically-derived surface normal (the cross product of the patch's
// two tangent directions, dP/du and dP/dv), so there's no nearest-neighbor
// search or Jacobi eigensolve anywhere in this file. See UtahTeapotLoaderB
// for the version that reuses this same baked point cloud -- plus the
// patch grid's already-known quad connectivity -- to fill in real
// Gouraud-shaded triangular faces instead of isolated points.

public class UtahTeapotLoaderA extends Loader {
    ...
    // spins steadily around a single upright axis, the same convention
    // SuzanneTheMonkeyLoader / StanfordDragonLoader / StanfordBunnyLoader
    // all use. (The previous version of this loader instead advanced both
    // angleX and angleY every frame, which tumbled the teapot end over
    // end rather than spinning it in place.)
    ...
    public UtahTeapotLoaderA(StatusStage[] stages, int width, int height) {
        super(stages, width, height);
        this.width = width;
        this.height = height;
    }

    public UtahTeapotLoaderA() {
    ...
    protected void renderGeometry(String[] outputBuffer, double[] zBuffer) {
        ...
        for (int i = 0; i < MESH_POSITIONS.length; i++) {
            ...
            projectTeapotVertex(p[0], p[1], p[2], n[0], n[1], n[2],
                    cosX, sinX, cosY, sinY, lightX, lightY, lightZ, outputBuffer, zBuffer);
        }
    }

    private void projectTeapotVertex(double lx, double ly, double lz, double nx, double ny, double nz,
